from helical_pdqueiros.settings import PROCESSED_DATA_ERROR_PATH, CHUNKED_DATA_PATH, CHUNKED_DATA_ERROR_PATH, PROCESSED_DATA_PATH, LOCAL_PROCESSED_DATA_PATH
...
class FineTuning(BaseTask):
    def __init__(self):
        super().__init__()

    def download_data_to_fine_tune(self) -> list[str]:
        return self.download_data(s3_input_folder=PROCESSED_DATA_PATH, local_output_folder=LOCAL_PROCESSED_DATA_PATH)

    def fine_tune(self) -> list[str]:
        res = []
        fine_tuner = CellTypeAnnotationFineTuner()
        res = []
        for file_name in os.listdir(LOCAL_PROCESSED_DATA_PATH):
            local_file_path = os.path.join(LOCAL_PROCESSED_DATA_PATH, file_name)
            res.append(local_file_path)
        fine_tuner.run(chunk_paths=res)
