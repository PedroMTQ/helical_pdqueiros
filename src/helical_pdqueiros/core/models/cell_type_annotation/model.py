...
class GeneformerFineTuningModel(HelicalBaseFineTuningModel, Geneformer):
    ...
    def _forward(
        ...
        """
        outputs = self.model(input_ids=input_ids, attention_mask=attention_mask_minibatch, return_dict=True)
        print(outputs.logits.dtype)
        intermediate_tensor = outputs.hidden_states[1]

        print(f"Intermediate Hidden State Dtype: {intermediate_tensor.dtype}")
        raise Exception
    ...
    def train(
        ...
        for j in range(epochs):
            ...
            for i in training_loop:
                ...
                print('MAX_RANGE', max_range, minibatch)
